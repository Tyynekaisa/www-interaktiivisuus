...
const itemToSee = document.querySelector("#itemtosee");
const inventory = document.querySelector("#backpack");
...
const winningbox = document.querySelector("#winningbox");
...
function render() {
    if (mapLocation == 9) {
        ...
        itemToSee.style.display = "none";
        inventory.style.display = "none";
        winningbox.style.display = "block";

    } else {
        
        output.innerHTML = `<b>Sijaintisi on:</b> ${map[mapLocation]}<br>`;
        itemToSee.innerHTML = "<b>Näet esineen:</b>";
        inventory.innerHTML = "<b>Reppusi sisältö:</b>"


        for (let i = 0; i < items.length; i++) {
            if (mapLocation === itemLocations[i]) {
                itemToSee.innerHTML += `<br>${items[i]}`;
            } else {
                itemToSee.innerHTML += `<br>Täällä ei näyttäisi olevan mitään.`;
            }
        }


        if (backPack.length !== 0) {
            inventory.innerHTML += `<br>${backPack.join(", ")}`;
        } else {
            inventory.innerHTML += `<br>Reppu on tyhjä.`;
        }
...
function playGame() {
    ...
    // Tarkista pelaajan syöte, onko käytettävä esine repussa
    item = "";
    for (let i = 0; i < backPack.length; i++) {
        if (playersInput.includes(backPack[i])) {
            item = backPack[i];
            console.log(`Haluttu esine repussa: ${item}`);
            break;
        }
    }

    // Tarkista pelaajan syöte, onko poimittava esine ruudussa
    itemToPick = "";
    for (let i = 0; i < knownItems.length; i++) {
        if (playersInput.indexOf(knownItems[i]) !== -1) {
            itemToPick = knownItems[i];
            console.log(`Haluttu esine: ${itemToPick}`);
            break;
        }
    }
...
function takeItem() {
    let itemIndexNumber = items.indexOf(itemToPick);

    if(itemIndexNumber !== -1 && itemLocations[itemIndexNumber] === mapLocation) {
        backPack.push(itemToPick);
        items.splice(itemIndexNumber, 1);
        itemLocations.splice(itemIndexNumber, 1);
        gameMessage += `Poimit esineen: ${itemToPick}`;
        ...
        if (mapLocation == 1 && backPack.includes("huilu")) { // Otetaan huilu kaivolta
            glueMessages[mapLocation] = "Ei enempää vihjeitä täällä!";
        }
...
function leaveItem() {
    if (backPack.length !== 0) {
        ...
        if (backPackIndexNumber !== -1) {
            ...
            if (item == "sävel") {
                gameMessage += `Sävel on ikuisesti muistissasi. Et voi jättää sitä.`
    ...
    switch(item) {
        case "huilu":
            if (mapLocation === 0) { // Tornilla
                ...
                } else if (backPack.includes("glittermaali") || items.includes("glittermaali")) {
            ...
            } else if (mapLocation === 7 && troubadour === true) { // Puupenkki ja trubaduuri
                if (backPack.includes("sävel") == false) { //säveltä ei ole vielä repussa
                    backPack.push("sävel");
                }
        ...
        case "sävel":
            if (mapLocation === 3) { // Lohikäärme
                gameMessage += "Kokeile soittaa sävel huilullasi!";
            } else {
                gameMessage += "Hyräilet säveltä mielessäsi.";
            }
            break;
